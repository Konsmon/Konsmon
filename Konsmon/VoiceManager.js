...
class VoiceManager {
    ...
    getVoiceNick() {
        return this.auth.getDisplayNick();
    }
    ...
    leaveVoiceChat(wasKicked = false) {
        ...
        if (this.state.voicePresenceRef) {
            this.state.voicePresenceRef.off();
            if (!wasKicked) this.state.audioDisconnect.play().catch(() => {});
    ...
    async _startMicTest(deviceId) {
        ...
        try {
            ...
            const draw = () => {
                if (!document.getElementById('micTestFill')) return;
