...
private:
    ...
    // When the output name is "-" the capture is streamed as raw PCM on stdout
    // instead of being written to a .wav file, so a host process can consume it live.
    bool m_bStreamToStdout = false;
