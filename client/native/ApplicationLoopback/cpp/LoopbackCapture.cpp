    ...
    m_activateResult = SetDeviceStateErrorIfFailed([&]()->HRESULT
        {
            ...
            // Creates the WAV file, or opens stdout for live streaming.
            RETURN_IF_FAILED(m_bStreamToStdout ? CreateStdoutStream() : CreateWAVFile());
...
HRESULT CLoopbackCapture::CreateWAVFile()
{
    return SetDeviceStateErrorIfFailed([&]()->HRESULT
        {
            // FILE_SHARE_READ lets a host process tail the file while capture is in progress.
...
//  CreateStdoutStream()
//
//  Prepares stdout to receive a one-line JSON format descriptor followed by raw PCM frames.
//  This is used instead of a .wav file so a host process can consume the capture live.
//
HRESULT CLoopbackCapture::CreateStdoutStream()
{
    return SetDeviceStateErrorIfFailed([&]()->HRESULT
        {
            HANDLE hStdout = GetStdHandle(STD_OUTPUT_HANDLE);
            RETURN_HR_IF(E_HANDLE, hStdout == nullptr || hStdout == INVALID_HANDLE_VALUE);

            // Duplicate the handle so m_hFile's destructor does not close the real stdout.
            HANDLE hDuplicate = nullptr;
            RETURN_IF_WIN32_BOOL_FALSE(DuplicateHandle(GetCurrentProcess(), hStdout, GetCurrentProcess(),
                &hDuplicate, 0, FALSE, DUPLICATE_SAME_ACCESS));
            m_hFile.reset(hDuplicate);

            char header[128] = {};
            int length = _snprintf_s(header, _TRUNCATE,
                "{\"sampleRate\":%lu,\"channels\":%u,\"bitsPerSample\":%u,\"encoding\":\"Pcm\"}\n",
                m_CaptureFormat.nSamplesPerSec, m_CaptureFormat.nChannels, m_CaptureFormat.wBitsPerSample);
            RETURN_HR_IF(E_UNEXPECTED, length <= 0);

            DWORD dwBytesWritten = 0;
            RETURN_IF_WIN32_BOOL_FALSE(WriteFile(m_hFile.get(), header, static_cast<DWORD>(length), &dwBytesWritten, NULL));
...
{
    // A stdout pipe carries raw PCM and cannot be seeked, so there is no header to fix up.
    if (m_bStreamToStdout)
    {
        FlushFileBuffers(m_hFile.get());
        return S_OK;
    }
...
HRESULT CLoopbackCapture::StartCaptureAsync(DWORD processId, bool includeProcessTree, PCWSTR outputFileName)
{
    ...
    m_bStreamToStdout = (outputFileName != nullptr) && (wcscmp(outputFileName, L"-") == 0);
