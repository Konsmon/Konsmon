...
function getPingedChatsForCurrentUser() {
    const set = new Set();
    if (!currentUser || !currentUser.uid) return set;
    Object.entries(chatsCache || {}).forEach(([chatId, chatData]) => {
        if (chatData && chatData.pings && chatData.pings[currentUser.uid]) {
            set.add(chatId);
        }
    });
    return set;
...
function renderServerList(filter) {
    ...
    const pingedChats = getPingedChatsForCurrentUser();
        ...
        if (isExpanded || filter) {
            ...
            if (server.channels && server.channels.text) {
                Object.entries(server.channels.text).forEach(([channelId, channelData]) => {
                    ...
                    const channelHasPing = pingedChats.has(channelId);
                    chanDiv.innerHTML = `<span class="tree-prefix">|_</span><span style="opacity:0.7">#</span> ${escapeHtml(channelData.name)}${channelHasPing ? '<span class="ping-dot" title="Mention"></span>' : ''}`;
...
function joinChat(id) {
    ...
    messagesRef.on('child_added', snap => {
        ...
        // build meta with separate time and nick spans so we can style registered users
        meta.textContent = '';
